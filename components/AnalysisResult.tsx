...
import type { SajuInfo, Pillar, Ohaeng, DaewoonPillar, SewoonPillar, SajuAnalysisResult } from '../types';
...
import { DiagnosisIcon, PrescriptionIcon, PrognosisIcon, SparklesIcon, UserIcon, ChevronDownIcon, HomeIcon } from './icons';
...
const CharBox: React.FC<{ char: string }> = ({ char }) => {
    ...
    return (
        <div className={`inline-flex items-center justify-center w-10 h-10 md:w-12 md:h-12 text-2xl font-bold rounded shadow-md ${color.bg} ${color.text} ${color.border ?? ''} saju-char-outline-small`}>
            {char}
        </div>
    );
};

const OhaengDisplayItem: React.FC<{ char: string; count: number }> = ({ char, count }) => {
    ...
    return (
        <div className="mt-4 pt-4 border-t border-gray-200 text-center text-lg font-bold">
             <div className="flex flex-wrap justify-center items-center gap-x-4 gap-y-2">
                <div className="flex items-center gap-1"><strong className="text-gray-500 font-bold">오행:</strong></div>
                ...
                <div className="flex items-center gap-2">
                 ...
                 <div className="flex items-center gap-2">
                    ...
                    {cheonEulGwiInCharsArray.length > 0 ? (
                        <div className="flex items-center gap-1.5">
                            {cheonEulGwiInCharsArray.map(char => <CharBox key={char} char={char} />)}
                        </div>
                    ) : (
...
const SajuPillarsDisplay: React.FC<{ sajuInfo: SajuInfo }> = ({ sajuInfo }) => {
    ...
    const renderPillar = (pillar: Pillar) => {
        ...
        return (
            <div key={pillar.label} className="flex flex-col text-center text-sm md:text-base">
                <div className="font-bold text-gray-700 py-2.5">{pillar.label}<span className="font-normal text-gray-400">({pillar.ganji})</span></div>
                ...
                <div className="flex justify-center py-1.5 px-2">
                    <div className={`saju-char-outline w-16 h-16 md:w-20 md:h-20 flex items-center justify-center text-4xl md:text-5xl font-bold rounded-lg shadow-lg ${ganColor.bg} ${ganColor.text} ${ganColor.border ?? ''}`}>
                        {pillar.cheonGan.char}
                    </div>
                </div>

                <div className="flex justify-center py-1.5 px-2">
                     <div className={`saju-char-outline w-16 h-16 md:w-20 md:h-20 flex items-center justify-center text-4xl md:text-5xl font-bold rounded-lg shadow-lg ${jiColor.bg} ${jiColor.text} ${jiColor.border ?? ''}`}>
                        {pillar.jiJi.char}
                    </div>
                </div>

                <div className="py-2 h-14 flex items-center justify-center font-semibold text-gray-700 text-base saju-text-outline">{pillar.jiJi.sibsin.name}</div>
                ...
                <div className="py-2 font-semibold text-gray-700 saju-text-outline">{pillar.jiJi.unseong.name}
...
const DaewoonDisplay: React.FC<{ sajuInfo: SajuInfo }> = ({ sajuInfo }) => {
    ...
    const renderDaewoonPillar = (pillar: DaewoonPillar) => {
        ...
        return (
            <div key={pillar.age} className={`flex flex-col text-center text-xs md:text-sm p-1.5 bg-gray-900/5 rounded-lg border-2 shadow-md flex-shrink-0 w-[80px] md:w-[90px] ${isActive ? 'border-yellow-500' : 'border-gray-200'}`}>
                <div className={`font-bold py-1 saju-text-outline ${isActive ? 'text-yellow-600' : 'text-gray-800'}`}>{pillar.age}세<span className="block text-xs font-normal text-gray-800">({pillar.ganji})</span></div>
                ...
                <div className="flex justify-center py-0.5">
                    <div className={`saju-char-outline-small w-10 h-10 md:w-12 md:h-12 flex items-center justify-center text-2xl md:text-3xl font-bold rounded-md shadow-md ${ganColor.bg} ${ganColor.text} ${ganColor.border ?? ''}`}>
                        {pillar.cheonGan.char}
                    </div>
                </div>

                <div className="flex justify-center py-0.5">
                    <div className={`saju-char-outline-small w-10 h-10 md:w-12 md:h-12 flex items-center justify-center text-2xl md:text-3xl font-bold rounded-md shadow-md ${jiColor.bg} ${jiColor.text} ${jiColor.border ?? ''}`}>
                        {pillar.jiJi.char}
                    </div>
                </div>
                
                <div className="py-1 h-9 flex items-center justify-center font-semibold text-gray-800 text-xs md:text-sm saju-text-outline">{pillar.jiJi.sibsin.name}</div>
    ...
    return (
        <div className="mt-8 p-4 md:p-6 glass-card">
            <h3 className="text-xl font-bold text-gray-800 mb-4 text-center">
                대운의 흐름 <span className="text-base font-medium text-gray-500">(한국 나이 기준)</span>
                <span className="block text-sm font-normal text-gray-500 mt-1">현재 나이: {koreanAge}세 | 대운 방향: {daewoon === 'sunhaeng' ? '순행' : '역행'}</span>
            </h3>
...
const SewoonDisplay: React.FC<{ sajuInfo: SajuInfo }> = ({ sajuInfo }) => {
    ...
    const sewoonPillars = useMemo(() => getSewoonPillars(currentYear, 10, ilGan), [currentYear, ilGan]);
    ...
    const renderSewoonPillar = (pillar: SewoonPillar) => {
        ...
        return (
            <div key={pillar.year} className={`flex flex-col text-center text-xs md:text-sm p-1.5 bg-gray-900/5 rounded-lg border-2 flex-shrink-0 w-[80px] md:w-[90px] shadow-md ${pillar.year === currentYear ? 'border-yellow-500' : 'border-gray-200'}`}>
                <div className={`font-bold py-1 saju-text-outline ${pillar.year === currentYear ? 'text-yellow-600' : 'text-gray-800'}`}>{pillar.year}년<span className="block text-xs font-normal text-gray-800">({pillar.ganji})</span></div>
...
const IlganPersonalityDisplay: React.FC<{ ilganChar: string }> = ({ ilganChar }) => {
    ...
    // 오행 색상 적용 (없을 경우 기본값)
    const ganColor = ganInfo ? ohaengColorMap[ganInfo.ohaeng] : { bg: 'bg-white', text: 'text-gray-900', border: 'border border-gray-200' };
                ...
                <div className="glass-card p-6 md:p-8 w-full max-w-3xl border-t-4 border-yellow-400 animate-fade-in-fast">
                    <div className="text-center mb-6">
                        ...
                        <h3 className="text-3xl font-extrabold text-gray-800 flex items-center justify-center gap-3">
                            {/* 오행 색상 적용된 박스 */}
                            <div className={`saju-char-outline w-12 h-12 flex items-center justify-center text-3xl rounded shadow-sm ${ganColor.bg} ${ganColor.text} ${ganColor.border ?? ''}`}>
                                {data.char}
                            </div>
                            <span>{data.name}</span>
                        </h3>
                        <p className="text-lg text-gray-600 mt-2 font-medium">"{data.nature}"</p>
                    </div>

                    <div className="mb-6 bg-white/50 p-4 rounded-xl border border-gray-100">
                        <h4 className="text-xl font-bold text-center text-amber-600 mb-3">"{data.title}"</h4>
                    ...
                    <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-200 text-center">
                        ...
                        <p className="text-gray-800 font-medium word-keep-all">{data.advice}</p>
                    </div>
                </div>
            )}
        </div>
...
const IljuAnalysisDisplay: React.FC<{ iljuGanji: string }> = ({ iljuGanji }) => {
    const [showInfo, setShowInfo] = useState(false);
                ...
                <div className="glass-card p-6 md:p-8 w-full max-w-3xl border-t-4 border-emerald-400 animate-fade-in-fast">
                    <div className="text-center mb-6">
                        ...
                        <h3 className="text-3xl font-extrabold text-gray-800 flex items-center justify-center gap-3">
                            <div className="flex gap-1">
                                <div className={`saju-char-outline w-10 h-10 flex items-center justify-center text-2xl rounded shadow-sm ${ganColor.bg} ${ganColor.text} ${ganColor.border ?? ''}`}>
                                    {gan}
                                </div>
                                <div className={`saju-char-outline w-10 h-10 flex items-center justify-center text-2xl rounded shadow-sm ${jiColor.bg} ${jiColor.text} ${jiColor.border ?? ''}`}>
                                    {ji}
                                </div>
                            </div>
                            <span>{data.name}</span>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-200 text-center">
                            <h5 className="font-bold text-emerald-800 mb-2">🍀 족집게 조언</h5>
                            <p className="text-gray-800 font-medium word-keep-all">{data.advice}</p>
                        </div>
                    </div>
                </div>
...
const SibsinPositionDisplay: React.FC<{ sajuInfo: SajuInfo }> = ({ sajuInfo }) => {
    const [showInfo, setShowInfo] = useState(false);
    const { pillars } = sajuInfo;
    ...
    return (
        <div className="mt-10 flex flex-col items-center animate-fade-in">
            <div className="text-center mb-6">
                ...
                <h3 className="text-xl font-bold text-gray-800 mt-1">월령과 일지 십신</h3>
            </div>
                ...
                <div className="glass-card p-6 md:p-8 w-full max-w-4xl border-t-4 border-purple-400 animate-fade-in-fast">
                    ...
                    <div className="space-y-10">
                        ...
                        <div className="bg-gradient-to-br from-blue-50 via-white to-blue-50 p-6 rounded-2xl border-2 border-blue-300 shadow-lg">
                            <div className="flex items-center justify-between mb-6 pb-4 border-b-2 border-blue-200">
                                <div className="flex items-center gap-3">
                                    <div className="bg-blue-500 text-white px-3 py-1.5 rounded-lg font-bold text-sm">월령 (月令)</div>
                                    <h4 className="text-2xl font-bold text-blue-900">{wollyeongSibsin}</h4>
                            ...
                            {sibsinDescriptions[wollyeongSibsin] && (
                                <div className="bg-white/80 p-5 rounded-xl mb-5 border border-blue-200">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        {sibsinDescriptions[wollyeongSibsin].keywords.map((kw, idx) => (
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-1.5 mb-3">
                                                {sibsinPositionDescriptions[wollyeongSibsin]['월주'].keywords.map((kw, idx) => (
                                                    <span key={idx} className="px-2 py-0.5 bg-blue-200 text-blue-900 rounded-full text-xs font-semibold">
                                                        {kw}
                                                    </span>
                                                ))}
                                            </div>
                        ...
                        <div className="bg-gradient-to-br from-pink-50 via-white to-pink-50 p-6 rounded-2xl border-2 border-pink-300 shadow-lg">
                            <div className="flex items-center justify-between mb-6 pb-4 border-b-2 border-pink-200">
                                <div className="flex items-center gap-3">
                                    <div className="bg-pink-500 text-white px-3 py-1.5 rounded-lg font-bold text-sm">일지 (日支)</div>
                                    <h4 className="text-2xl font-bold text-pink-900">{iljiSibsin}</h4>
                                ...
                                <div className="bg-gradient-to-r from-pink-100/50 to-white p-5 rounded-xl border border-pink-300">
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-1.5 mb-3">
                                                {sibsinPositionDescriptions[iljiSibsin]['일주'].keywords.map((kw, idx) => (
                                                    <span key={idx} className="px-2 py-0.5 bg-pink-200 text-pink-900 rounded-full text-xs font-semibold">
                                                        {kw}
                                                    </span>
                                                ))}
                                            </div>
                    ...
                    <div className="mt-8 p-5 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl border-2 border-purple-200 text-center">
                        ...
                        <p className="text-purple-800 text-sm leading-relaxed">
                            <strong>월령(月令)</strong>은 사회적 성공과 직업운을, <strong>일지(日支)</strong>는 배우자와의 관계 및 가정의 행복을 결정짓는 핵심 요소입니다.
                            이 두 가지를 이해하면 인생의 큰 그림을 볼 수 있습니다.
                        </p>
                    </div>
                </div>
            )}
...
const renderResult = (text: string) => {
    ...
    return lines.map((line, index) => {
        ...
        if (line.startsWith('## ')) {
            return <h2 key={index} className="text-3xl font-bold mt-8 mb-4 pb-2 border-b-2 border-gray-200">{line.replace('## ', '')}</h2>;
        }
        if (line.startsWith('**') && line.endsWith('**')) {
             return <h4 key={index} className="text-xl font-semibold mt-6 mb-2 text-amber-600">{line.replace(/\*\*/g, '')}</h4>;
        }
        ...
        return <p key={index} className="my-3 text-base leading-relaxed">{renderedLine}</p>;
    });
};
...
const SajuImageDisplay: React.FC<{
    ...
    return (
        <div className="my-10 animate-fade-in">
            <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">사주 심상 (四柱 心象): 시각화</h2>
            <div className="glass-card p-4 md:p-6 flex items-center justify-center min-h-[300px] aspect-square max-w-lg mx-auto">
                {isLoading && (
                    <div className="w-full h-full flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-gray-500 mt-4">일주를 상징하는 이미지를 생성 중입니다...</p>
                    </div>
                )}
                {error && (
...
export const AnalysisResult: React.FC<AnalysisResultProps> = ({ result, sajuData, isLoading, sajuImage, isImageLoading, imageError }) => {
    ...
    const stageInfo = useMemo(() => result ? [
        { 
            id: 'stage1', 
            title: '1단계: 일주와 오행 분석', 
    ...
    const birthDateString = `${birthDate.year}년 ${birthDate.month}월 ${birthDate.day}일 ${String(birthDate.hour).padStart(2, '0')}:${String(birthDate.minute).padStart(2, '0')}`;
                ...
                <div className="mb-8 p-4 md:p-6 glass-card">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                        <div className="p-2">
                            ...
                            <div className="font-semibold text-gray-800 text-lg">{gender === 'male' ? '남성' : '여성'}</div>
                        </div>
                        <div className="p-2">
                            <div className="text-gray-500 text-sm mb-1">대운 방향</div>
                            <div className="font-semibold text-gray-800 text-lg">{daewoon === 'sunhaeng' ? '순행' : '역행'}</div>
                        </div>
                        <div className="p-2">
                            <div className="text-gray-500 text-sm mb-1">대운수</div>
                            <div className="font-semibold text-gray-800 text-lg">{daewoonNumber}세</div>
                        </div>
                        <div className="p-2">
                            <div className="text-gray-500 text-sm mb-1">출생 지역</div>
                            <div className="font-semibold text-gray-800 text-lg">{birthRegion}</div>
                        </div>
                        <div className="col-span-2 sm:col-span-4 p-2 mt-4 border-t border-gray-200">
                            ...
                            <div className="font-semibold text-gray-800 text-lg">{birthDateString}</div>
                        </div>
                    </div>
                </div>
                ...
                {result && !showAiDetails && (
                    <div className="mt-12 flex justify-center animate-fade-in">
                        ...
                        >
                            ...
                            <span className="text-lg font-bold">AI 심층 분석 결과 전체 보기</span>
            ...
            {showAiDetails && (isLoading || result) && (
                <div className="animate-fade-in">
                    <div className="mt-12 border-t-2 border-gray-100 pt-10">
                        <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">사주 분석 상세 결과</h2>
                        <div className="space-y-8">
                            {isLoading && !result ? (
                                ...
                                stageInfo.map(stage => (
                                    stage.content && (
                                        <div key={stage.id} className={`glass-card p-6 md:p-8 overflow-hidden relative border ${stage.borderColor}`}>
                                            <div className="flex items-start gap-5">
                                                ...
                                                <div className="flex-grow">
                                                    <h3 className="text-2xl font-bold text-gray-900 mb-4">{stage.title}</h3>
