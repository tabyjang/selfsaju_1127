...
const yangUnseongStartIndex: { [key: string]: number } = {
  甲: 11,
  丙: 2,
  戊: 2,
  庚: 5,
...
const yinUnseongStartIndex: { [key: string]: number } = {
  乙: 6,
  丁: 9,
  己: 9,
  辛: 0,
...
const getUnseong = (ilgan: string, targetJiji: string) => {
  ...
  const startIndex = ilganIsYang
    ? yangUnseongStartIndex[ilgan]
    : yinUnseongStartIndex[ilgan];
  ...
  return {
    name: unseongCycleNames[unseongIndex],
    hanja: unseongCycleHanja[unseongIndex],
  };
};
...
export const getSajuInfoFromCharacters = (
  ...
  const createPillar = (label: string, gan: string, ji: string): Pillar => {
    ...
    const ganInfo =
      gan === "-"
        ? { ohaeng: "earth" as Ohaeng, yinYang: "yang" as YinYang }
        : earthlyBranchGanInfo[gan];
    const jiInfo =
      ji === "-"
        ? { ohaeng: "earth" as Ohaeng, yinYang: "yang" as YinYang }
        : earthlyBranchGanInfo[ji];

    const cheonGan: Gan = {
      char: gan,
      ohaeng: ganInfo.ohaeng,
      sibsin:
        gan === "-"
          ? { name: "-", hanja: "-" }
          : isIlju
    ...
    const jiJi: Ji = {
      char: ji,
      ohaeng: jiInfo.ohaeng,
      sibsin: ji === "-" ? { name: "-", hanja: "-" } : getSibsin(ilGan, ji),
      jijanggan:
        ji === "-"
          ? []
          : (jijangganData[ji] || []).map((jigan) => ({
              char: jigan,
              ohaeng: earthlyBranchGanInfo[jigan].ohaeng,
              sibsin: getSibsin(ilGan, jigan),
            })),
      unseong: ji === "-" ? { name: "-", hanja: "-" } : getUnseong(ilGan, ji),
    };
  ...
  const daewoonPillars = getDaewoonPillars(
    wolJuGanji,
    daewoon,
    daewoonNumber,
    ilGan
  ...
  // 시간이 모름인 경우 시주를 null로 처리하거나 빈 값으로 처리
  const hourPillar =
...
export const getSajuFromDate = (
  ...
  const adjustedBirthDate = new Date(
    birthDate.getTime() - timeOffsetMinutes * 60 * 1000
  );
  ...
  for (let i = 0; i < 12; i++) {
    if (
      adjustedBirthDate >= jeolgiDates[i] &&
      adjustedBirthDate < jeolgiDates[i + 1]
    ) {
  ...
  if (yearStemIndex === 0 || yearStemIndex === 5)
    monthStemStartForInWol = 2; // 갑기 -> 병
  else if (yearStemIndex === 1 || yearStemIndex === 6)
    monthStemStartForInWol = 4; // 을경 -> 무
  else if (yearStemIndex === 2 || yearStemIndex === 7)
    monthStemStartForInWol = 6; // 병신 -> 경
  else if (yearStemIndex === 3 || yearStemIndex === 8)
    monthStemStartForInWol = 8; // 정임 -> 임
  else monthStemStartForInWol = 0; // 무계 -> 갑

  const monthStemIndex =
    (monthStemStartForInWol + monthBranchIndexInCycle) % 10;
  ...
  const daysDiff = Math.floor(
    (birthDateUTC - refDateUTC) / (1000 * 60 * 60 * 24)
  );
  ...
  // --- 4. 시주 (Hour Pillar) 계산 ---
  let timeStem: string;
  let timeBranch: string;

  if (isHourUnknown) {
    // 시간이 모름인 경우, 시주를 빈 값으로 설정
    timeStem = "";
    timeBranch = "";
  } else {
    // 시주 계산 시 야자시 적용 여부와 관계 없이 KST 23:00~00:59는 자시(子時)로 처리
    const timeBranchIndex =
      originalHour === 23 ? 0 : Math.floor((originalHour + 1) / 2) % 12;
    timeBranch = earthlyBranches[timeBranchIndex];
    ...
    const hourStemStartMap: { [key: number]: number } = {
      0: 0,
      5: 0, // 甲(0), 己(5)일 -> 甲子(0)시 부터 시작
      1: 2,
      6: 2, // 乙(1), 庚(6)일 -> 丙子(2)시 부터 시작
      2: 4,
      7: 4, // 丙(2), 辛(7)일 -> 戊子(4)시 부터 시작
      3: 6,
      8: 6, // 丁(3), 壬(8)일 -> 庚子(6)시 부터 시작
      4: 8,
      9: 8, // 戊(4), 癸(9)일 -> 壬子(8)시 부터 시작
    };

    const timeStemStartForJaSi = hourStemStartMap[dayStemIndex];
    if (timeStemStartForJaSi === undefined) {
      throw new Error(
        `Could not determine hour stem start for day stem index: ${dayStemIndex}`
      );
    }

    const timeStemIndex = (timeStemStartForJaSi + timeBranchIndex) % 10;
    timeStem = heavenlyStems[timeStemIndex];
  }
  ...
  // 양년생 남자, 음년생 여자는 순행
  if (
    (isYearStemYang && gender === "male") ||
    (!isYearStemYang && gender === "female")
  ) {
    daewoon = "sunhaeng";
  } else {
    daewoon = "yeokhaeng";
  ...
  if (daewoon === "sunhaeng") {
    if (!nextTermDate)
      throw new Error("대운 계산을 위한 다음 절기를 찾을 수 없습니다.");
    diff = nextTermDate.getTime() - adjustedBirthDate.getTime();
  } else {
    if (!currentTermDate)
      throw new Error("대운 계산을 위한 이전 절기를 찾을 수 없습니다.");
